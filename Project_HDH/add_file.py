...
def delete():
    ...
    try:
        if os.path.exists(path):
            os.rmdir(path)
            lb3.configure(text='Xóa thư mục thành công')
    except OSError:
        lb3.configure(text='Error: Đường dẫn không tồn tại!')
...
deletebtn = Button(frame3,text='Delete',command=delete).place(x=240,y=120)

exitbtn = Button(frame3,text='Exit',command=exit).place(x=400,y=120)
